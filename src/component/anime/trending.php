...
document.addEventListener("DOMContentLoaded", function() {
    ...
    var swiper = new Swiper('#anime-trending .swiper-container', {
        slidesPerView: 'auto',
        spaceBetween: 15,
        navigation: {
            nextEl: '#anime-trending .navi-next',
            prevEl: '#anime-trending .navi-prev'
        },
        freeMode: true
    });

    // Bind update on slide and transition
    swiper.on('init', updateLanguageDisplay);
    swiper.on('slideChange', updateLanguageDisplay);
    swiper.on('transitionEnd', updateLanguageDisplay);

    // Run immediately for initial
